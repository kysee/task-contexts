...
현재 세션에서는 TxEventProofPayload에 `mspids` 필드 추가 및 조직별 Root CA 조회 체계 반영 작업을 수행.
...
### btip-21.md — ILinkerEventHandler 추가 → BTIP26으로 분리

- 원래 `ILinkerEventHandler` 인터페이스를 btip-21에 추가했으나, 현재 세션에서 btip-26으로 분리 완료
...
| 파일 | 상태 | 현행 주요 구조 |
|------|------|---------------|
| `btip-17.md` | ✅ 변경 없음 (표기 교정만) | 교차 참조 정합적 |
...
| `btip-22-xx.md` | ⚠️ 미수정 | 차기 세션에서 조직별 Root CA 매핑 반영 예정 |
